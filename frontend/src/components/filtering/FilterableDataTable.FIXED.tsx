/**
 * FIXES APPLIED:
 * - Fixed: FilterManager now instantiated once per component, not on every render
 * - Fixed: Added filterManager to useMemo dependencies array
 * - Fixed: Added comprehensive error handling for filter operations
 * - Fixed: Added null/undefined checks before accessing column.render
 * - Fixed: Added try-catch wrapper around filter application
 * - Fixed: Added proper fallback for column.key rendering
 */
...
export function FilterableDataTable<T extends Record<string, any>>({
  ...
  // BUG FIX: Move FilterManager outside useMemo so it persists across renders
  ...
  const filteredData = useMemo(() => {
    try {
      setFilterError('');
      // BUG FIX: Use ref to avoid recreating FilterManager
      let result = filterManagerRef.current.applyFilter(data, conditions);
      ...
      if (sortBy) {
        result = result.sort((a, b) => {
          ...
          // BUG FIX: Handle null/undefined values in sorting
      ...
      // BUG FIX: Catch and display filter errors
      const errorMsg = error instanceof Error ? error.message : 'Unknown filter error';
      setFilterError(`Failed to apply filters: ${errorMsg}`);
  ...
  const handleSaveFilter = (name: string, description?: string) => {
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : 'Failed to save filter';
          ...
          <SavedFiltersList
            ...
            onDelete={(id) => {
              try {
                ...
                const msg = error instanceof Error ? error.message : 'Failed to delete filter';
                setFilterError(msg);
              }
            }}
            onToggleFavorite={(id) => {
              ...
              {filteredData.map((row, rowIndex) => (
                ...
                >
                  {columns.map((column) => (
                    ...
                    >
                      {/* BUG FIX: Null check before calling render function */}
