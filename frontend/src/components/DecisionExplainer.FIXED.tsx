/**
 * FIXES APPLIED:
 * - Fixed: Added error handling for navigate() call with fallback
 * - Fixed: Added null/undefined checks for all explanation property accesses
 * - Fixed: Improved logId parsing with parseInt error handling
 * - Fixed: Added abort controller for cleanup and race condition prevention
 * - Fixed: Added safety checks for nested properties (top_features, scores, etc.)
 * - Fixed: Improved error messages with proper type checking
 */
...
const DecisionExplainer = ({ logId: propLogId }: DecisionExplainerProps) => {
  ...
  // BUG FIX: Properly parse and validate logId
  let logId: number | null = null;
  if (propLogId) {
    logId = propLogId;
  } else if (paramLogId) {
    try {
      const parsed = parseInt(paramLogId, 10);
      if (!isNaN(parsed) && parsed > 0) {
        logId = parsed;
      }
    } catch {
      // Invalid logId, will be null
    }
  }
  ...
  useEffect(() => {
    ...
    // BUG FIX: Create abort controller for cleanup and race condition prevention
    ...
    const loadExplanation = async () => {
      try {
        ...
        // BUG FIX: Don't update state if request was cancelled
        if (abortControllerRef.current?.signal.aborted) return;
    ...
    // BUG FIX: Cleanup on unmount or logId change
    return () => {
      abortControllerRef.current?.abort();
    };
  }, [logId]);

  // BUG FIX: Safe navigate with error handling
    ...
    return (
      <Box sx={{ minHeight: '100vh', py: 3 }}>
        <Container maxWidth="lg">
          <Alert severity="error">Invalid or missing log ID</Alert>
        </Container>
      </Box>
    );
  }
        ...
        <Box sx={{ maxWidth: '900px' }}>
          ...
          {explanation && !loading && (
            <>
              ...
              <Box sx={{ mb: 4 }}>
                <Box sx={{ 
                  ...
                  '&::before': {
                    ...
                    height: '100%',
                    width: '6px',
                    // BUG FIX: Null check for decision
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 2, mb: 4 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                      ...
                      >
                        <IconifyIcon 
                          // BUG FIX: Safe decision check
                          icon={explanation.explanation?.decision === 'granted' ? 'mdi:check-circle' : explanation.explanation?.decision === 'denied' ? 'mdi:close-circle' : 'mdi:clock'} 
                          sx={{ fontSize: '3rem', color: getDecisionColor(explanation.explanation?.decision) }} 
                        />
                      </Box>
                      <Box>
                        <Typography variant="h3" sx={{ fontWeight: 800, mb: 0.5, letterSpacing: 1, color: '#fff' }}>
                          {/* BUG FIX: Safe decision access with fallback */}
                          {(explanation.explanation?.decision || 'UNKNOWN').toUpperCase()}
                        </Typography>
                    ...
                    {/* BUG FIX: Safe risk level access */}
                    <Chip
                      label={`RISK: ${(explanation.explanation?.risk_level || 'UNKNOWN').toUpperCase()}`}
                      sx={{
                  ...
                  <Box sx={{ mb: 4, p: 3, borderRadius: 3, bgcolor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <Typography variant="h6" sx={{ fontStyle: 'italic', lineHeight: 1.6, color: '#e2e8f0', fontWeight: 500 }}>
                      {/* BUG FIX: Safe reason access with fallback */}
                      "{explanation.explanation?.reason || 'No reasoning available'}"
                    </Typography>
                  </Box>
                  ...
                  <Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
                      ...
                      <Typography variant="body1" sx={{ fontWeight: 800, color: getDecisionColor(explanation.explanation?.decision) }}>
                        {/* BUG FIX: Safe confidence access with validation */}
                        {(explanation.explanation?.confidence != null ? explanation.explanation.confidence * 100 : 0).toFixed(1)}%
                      </Typography>
                    </Box>
                    <LinearProgress
                      variant="determinate"
                      // BUG FIX: Safe confidence access with bounds check
                      value={Math.min(100, Math.max(0, (explanation.explanation?.confidence ?? 0) * 100))}
                      sx={{
                        height: 10,
                        borderRadius: 5,
              ...
              {/* Features Section */}
              {/* BUG FIX: Safe array access and length check */}
              {Array.isArray(explanation.explanation?.top_features) && explanation.explanation.top_features.length > 0 && (
                <Box sx={{ mb: 4 }}>
                  <Box sx={{ 
                    p: 4, 
                    borderRadius: 4, 
                      ...
                      <TableBody>
                        {explanation.explanation.top_features.map((f, idx) => (
                          <TableRow key={idx} sx={{ '&:hover': { bgcolor: 'rgba(255,255,255,0.02)' }, transition: 'all 0.2s' }}>
                            {/* BUG FIX: Safe property access with fallback */}
                            <TableCell sx={{ fontSize: '1rem', fontWeight: 600, color: '#e2e8f0' }}>{f?.name ?? 'N/A'}</TableCell>
                            ...
                            <TableCell align="right" sx={{ fontSize: '1rem', fontWeight: 800, color: '#fbbf24' }}>
                              +{f?.contribution != null ? Number(f.contribution).toFixed(3) : '-'}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
