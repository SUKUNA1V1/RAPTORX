...
const DEFAULT_CARD_COLOR = "#111827";
...
const CardInner = ({ label, title, description, value, content }: CardInnerProps) => (
  <>
    <div className={styles.magicBentoCardHeader}>
      <div className={styles.magicBentoCardLabel}>{label}</div>
      {value ? <div className={styles.magicBentoCardValue}>{value}</div> : null}
    </div>
    <div className={`${styles.magicBentoCardContent} ${content ? styles.magicBentoCardContentCustom : ""}`}>
      {content ? (
        <div className={styles.magicBentoCardCustomInner}>{content}</div>
      ) : (
        <>
          <h2 className={styles.magicBentoCardTitle}>{title}</h2>
          <p className={styles.magicBentoCardDescription}>{description}</p>
        </>
      )}
    </div>
  </>
...
const StaticCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      if ((!clickEffect && !onCardClick) || shouldDisableAnimations) return;
  ...
  }, [clickEffect, enableMagnetism, enableTilt, glowColor, onCardClick, shouldDisableAnimations]);

  return (
    <div ref={elementRef} data-magic-bento-card="true" className={className} style={style}>
      {children}
    </div>
  );
};
...
const GlobalSpotlight = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      const section = gridRef.current.closest("[data-bento-section='true']") as HTMLElement | null;
...
const BentoCardGrid = ({ children, gridRef }: { children: React.ReactNode; gridRef: React.RefObject<HTMLDivElement | null> }) => (
  <div className={`${styles.cardGrid} ${styles.bentoSection}`} data-bento-section="true" ref={gridRef}>
    {children}
  </div>
);
